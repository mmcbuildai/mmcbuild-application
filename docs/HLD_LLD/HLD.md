...
- **Observability** `[V]` — three layers: (1) AI usage logged to `ai_usage_log` via `trackUsage` in the router; (2) an AI-metrics dashboard at `/settings/ai-performance`; (3) **Sentry error tracking now wired** (`@sentry/nextjs ^10.62.0`) — `src/instrumentation.ts` (`register` + `onRequestError`), `src/instrumentation-client.ts`, `src/sentry.{server,edge}.config.ts`, `src/app/global-error.tsx`, wrapped in `next.config.ts` via `withSentryConfig`. Init is a graceful no-op until `NEXT_PUBLIC_SENTRY_DSN` is set; `sendDefaultPii: false` (REGULATED — no PII to a third party); Session Replay deliberately off. Required env: `NEXT_PUBLIC_SENTRY_DSN` (runtime) + `SENTRY_ORG`/`SENTRY_PROJECT`/`SENTRY_AUTH_TOKEN` (build-time source-map upload; mark sensitive, prod+preview). (4) **Vercel Web Analytics wired** (`@vercel/analytics ^2.0.1`, `<Analytics />` in `src/app/layout.tsx`) — cookieless page/route metrics, no PII, auto-active on Vercel.
...
**Doc-discovered items — both now RESOLVED (2026-06-30, follow-up PR):**
- ~~Sentry observability not wired~~ → **wired** (`@sentry/nextjs`); ~~Vercel Analytics unwired~~ → **wired** (`@vercel/analytics`). See §8.
- ~~Inngest function naming drift in CLAUDE.md~~ → **corrected** (CLAUDE.md module map now lists `process-plan` / `run-cost-estimation` and the real Build/Direct/Train function sets).
...
- [x] `[V]` Observability — Sentry error tracking + Vercel Web Analytics wired; AI-metrics dashboard + `ai_usage_log` present. `[?]` environments/prod-access ownership not code-determinable.
...
**Still `[?]` (gaps):** environment list + prod-access ownership (operational, not in repo); Jira ticket states (not re-checked in a code pass). *(Observability fully wired — Sentry + Vercel Analytics — §8.)*
