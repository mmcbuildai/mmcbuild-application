...
- ~~Sentry observability not wired~~ → **wired** (`@sentry/nextjs`, see §8). Vercel Analytics remains unwired (lower priority).
...
- [x] `[V]` Observability — Sentry error tracking wired (no-op until DSN set); AI-metrics dashboard + `ai_usage_log` present. `[?]` Vercel Analytics still not wired; environments/prod-access ownership not code-determinable.
...
**Still `[?]` (gaps):** Vercel Analytics not wired (Sentry error tracking now is — §8); environment list + prod-access ownership (operational, not in repo); Jira ticket states (not re-checked in a code pass).
